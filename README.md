# PCD-ESCOM
Programacion para Ciencia de Datos course at ESCOM
---
Repositorio de retos semanales del curso. Cada reto resuelve un problema de procesamiento de datos, aplicando los conceptos vistos en clase de forma progresiva.

## Estructura
Cada reto se encuentra en una carpeta llamada ```reto-semana-<número de semana>```, mientras que las evaluaciones se organizan por parciales. Cada sección contiene el programa principal ```main.py``` y su respectiva documentación.


### Progreso


| Semana | Reto | Estado | Enlace |
| :---: | :--- | :---: | :--- |
| 01 |  1 Calculadora de sumas |  Finalizado | [Ver Reto](./reto-semana-01/README.md) |
| 02 |  2 Clasificador de temperaturas | Finalizado | [Ver Reto](./reto-semana-02/README.md) |
| 03 |  3 Analizador de ventas | Finalizado | [Ver Reto](./reto-semana-03/README.md) |
| 04 |  4 Sistema de inventario modular | Finalizado | [Ver Reto](./reto-semana-04/README.md) |
| EXAM | Examen 1: Sistema de Taller Mecanico | Finalizazo | [Ver Examen](./examen1_pcd/README.md)
| 05 |  5 Perfilador de datasets |  Finalizado | [Ver Reto](./reto-semana-05/README.md) |
| 06 |  6 Validador de Codigos (RegEx) | Finalizado | [Ver Reto](./reto-semana-06/README.md)|

**by** Roony Roldan Cruz  